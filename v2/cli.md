...
## Version reporting<a name="version-reporting"></a>
...
## Authentication with AWS<a name="cli-auth"></a>
...
If your app uses [CDK Pipelines](cdk-pipeline.md), the CDK CLI understands your stacks and stages as a hierarchy. Also, the --all option and the `*` wildcard only match top-level stacks. To match all the stacks, use `**`. Also use `**` to indicate all the stacks under a particular hierarchy.
...
If your application contains [CDK Pipelines](cdk-pipeline.md) stacks, the CDK CLI displays stack names as paths according to their location in the pipeline hierarchy. (For example, `PipelineStack`, `PipelineStack/Prod`, and `PipelineStack/Prod/MyService`.)
...
| build | The command that compiles or builds the CDK application before synthesis. Not permitted in \$1/.cdk.json. | --build | 
