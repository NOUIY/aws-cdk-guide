# Example: Create a CDK app with multiple stacks<a name="stack-how-to-create-multiple-stacks"></a>
...
This example assumes that all [getting started](getting-started.md) steps have been completed.
...
## Create two stack instances<a name="stack-how-to-create-multiple-stacks-create-stacks"></a>
...
To deploy this stack to our AWS environment, we can issue one of the following commands. The first command uses our default AWS profile to obtain the credentials to deploy the stack. The second uses a profile that we specify. For *PROFILE\$1NAME*, we can substitute the name of an AWS CLI profile that contains appropriate credentials for deploying to the `us-east-1` AWS Region.
